...
# also plot the difference of the 2 variables-What Kind of plot? Point or Density?
# make sure you adjust so both are in the same units#######NOT SURE WHAT THIS MEANS, RECHECK#########
```
...
mean(data_train$fracinf) #Seems a bit too easy, I think I might have misunderstood this one... Recheck later.
...
```{r}
set.seed(1111) #makes each code block reproducible-lm
...
resultmat_lm <- data.frame(Variable = names(data_train)[-1], RMSE = rep(0,Npred)) #store values for RMSE for each variable

for (n in 2:ncol(data_train)) #loop over each predictor. For this to work, outcome must be in 1st column
{
  fit1 <- train( as.formula(paste("fracinf ~",names(data_train)[n])) , data = data_train, method = "lm", trControl = fitControl) 
 resultmat_lm[n-1,2]= fit1$results$RMSE  
}
print(resultmat_lm)
...
set.seed(1111) #makes each code block reproducible-earth
...
Npred_earth <- ncol(data_train)-1 # number of predictors

resultmat_earth <- data.frame(Variable = names(data_train)[-1], RMSE = rep(0,Npred_earth)) #store values for RMSE for each variable

for (n in 2:ncol(data_train)) #loop over each predictor. For this to work, outcome must be in 1st column
{
  fit2 <- train( as.formula(paste("fracinf ~",names(data_train)[n])), data = data_train, method = "earth", trControl = fitControl) 
 resultmat_earth[n-1,2]= fit2$results$RMSE  
}
print(resultmat_earth)


```

```{r}
set.seed(1111) #makes each code block reproducible-knn

#write code that uses the train function in caret to fit the outcome to all predictors using the 3 methods specified.


fitControl_knn <- trainControl(method="repeatedcv",number=5,repeats=5) #setting CV method for caret

Npred <- ncol(data_train)-1 # number of predictors

resultmat_knn <- data.frame(Variable = names(data_train)[-1], RMSE = rep(0,Npred)) #store values for RMSE for each variable

for (n in 2:ncol(data_train)) #loop over each predictor. For this to work, outcome must be in 1st column
{
  fit3 <- train( as.formula(paste("fracinf ~",names(data_train)[n])) , data = data_train, method = "knn", trControl = fitControl_knn) 
 resultmat_knn[n-1,2]= fit3$results$RMSE  
}
print(resultmat_knn)
```
...
train_drop_near_zero <- dplyr::select(data_train, -Deaths, -Hemisphere, -Hospitalizations, -MeanD1, -MeanI1, -MedianI1)
